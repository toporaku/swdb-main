...
- [x] **INFRA-DATA-005**: The gateway-service config-data file shall expose Actuator endpoints (`health,info,metrics,loggers,threaddump,heapdump`) with `management.endpoint.health.show-details=always` for Spring Boot Admin monitoring.
